...
# Note: 
# The orininal version of this test used one interface that allows both AP and monitor mode.
# The intercace we are using doesnt support this, so I have modified the code to allow it to run using two physical interfaces.
# This will be used for testing the KRACK vulnerabilities in Wi-Fi clients however it is unlikely that you will find vulnerable clients.
# Ayman 
...
class KRAckAttackClient():
	def __init__(self):
		...
		# MODIFIED: Use second physical interface instead of creating virtual interface
		self.nic_mon = "wlan1"  # Change this to your second interface name
		self.options = None
		try:
	...
	def configure_interfaces(self):
		...
		# 2. Configure monitor mode on interfaces - MODIFIED to use two physical interfaces
		# Instead of creating virtual interface, put the second physical interface into monitor mode
	...
	def run(self, options):
		...
		self.dhcp = DHCP_sock(sock=self.sock_eth,
						...
						pool=Net('192.168.140.0/24'),
						network='192.168.140.0/24',
						gw='192.168.140.254',
						renewal_time=600, lease_time=3600)
		# Configure gateway IP: reply to ARP and ping requests
		subprocess.check_output(["ifconfig", self.nic_iface, "192.168.140.254"])
