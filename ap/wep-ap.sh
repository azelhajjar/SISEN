...
SSID="${SSID:-${SSID_PREFIX}-WEP-AP}"
CHANNEL="${CHANNEL:-6}"
WEP_KEY="${WEP_KEY:-AABBCCDDEE}"
...
docheckbefore() {
  ...
  sleep 1
  iw dev "$INTERFACE" set freq 2437 2>/dev/null || true
  sleep 1
  ip link set "$INTERFACE" down 2>/dev/null || true
...
start_services() {
  ...
  ok "WEP AP Enabled: SSID ${CYAN}${SSID}${RESET} on ${CYAN}${INTERFACE}${RESET} (${CYAN}${AP_IP_BASE}${RESET})"
...
tail_clean() {
  ...
  info "Note: WEP support on modern operating systems is limited."
