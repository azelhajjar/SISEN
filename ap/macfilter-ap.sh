...
# Extra files: files/allowed_macs.txt (one MAC per line)
# ─────────────────────────────────────────────────────────────────────────────
...
SSID_PREFIX="${SSID_PREFIX:-WIRELESS-LAB}"
SSID="${SSID:-${SSID_PREFIX}-MAC-AP}"
CHANNEL="${CHANNEL:-6}"
MAC_FILE="${MAC_FILE:-$COURSE_DIR/ap/files/allowed_macs.txt}"
...
docheckbefore() {
  if [ ! -f "$MAC_FILE" ]; then
    err "MAC list not found: $MAC_FILE"
    err "Create it with one MAC per line, e.g. 00:11:22:33:44:55"
    exit 2
  fi
